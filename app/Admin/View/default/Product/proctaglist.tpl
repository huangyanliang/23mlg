...
<block name="main">
 <div class="pubmain">
  <h1>类别管理<div class="pull-right">{:btn(array('vals'=>'添加类别','size'=>3,'icon'=>'plus','scene'=>'primary','url'=>U('Product/proctagadd','tables='.$dshow['table'].'&sty='.$sty)))}</div></h1>
  ...
  <tr class="maintr">
    ...
    <td align="center" valign="middle">{:btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','url'=>U('Product/proctagmod','tables='.$dshow['table'].'&id='.$obj['Id'])))}
    </td>
   </tr>
  </volist>
  <tr>
 ...
 <else/>
 <div class="alert alert-danger">暂无资料，点击右上角的添加按钮添加一条数据吧</div>
 </if>
 </div>
</block>
