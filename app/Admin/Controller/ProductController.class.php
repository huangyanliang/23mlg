...
class ProductController extends AdminCommonController{
    ...
    public function productadd(){
        ...
        if ($send == '') {
            ...
            if ($send == '提交') {
                $result = M($tables)->add($this->fieldArr(array('ctag', 'mtag', 'linkurl', 'intro', 'content', 'keyword', 'metades', 'date','istop', 'pic', 'sty'),array('atlas'=>$pics)));
                if ($result) {
    ...
    public function productmod(){
        ...
        if ($id != 0) {
            ...
            if ($save == '') {
                ...
                if ($save == '确定修改') {
                    $result = M($tables)->where(array('Id' => $id))->save($this->fieldArr(array('ctag', 'mtag', 'linkurl', 'intro', 'content', 'keyword', 'metades', 'date','istop', 'pic'),array('atlas'=>$pics)));
                    if ($result) {
                        $this->success('资料修改成功！', U('Product/productlist', 'tables=' . $tables . '&sty=' . $data['sty']));
                    } else {
