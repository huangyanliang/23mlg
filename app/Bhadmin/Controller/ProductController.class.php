...
class ProductController extends AdminCommonController{
    ...
    public function productadd(){
        ...
        if ($send == '') {
            ...
            if ($send == '提交') {
                $result = M($tables)->add($this->fieldArr(array('ctag', 'mtag', 'intro', 'content', 'keyword', 'metades', 'date','istop', 'pic', 'sty')));
                if ($result) {
    ...
    public function productmod(){
        ...
        if ($id != 0) {
            ...
            if ($save == '') {
                ...
                if ($save == '确定修改') {
                    $result = M($tables)->where(array('Id' => $id))->save($this->fieldArr(array('ctag', 'mtag', 'intro', 'content', 'keyword', 'metades', 'date','istop', 'pic')));
                    if ($result) {
