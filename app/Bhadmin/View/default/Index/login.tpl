...
<head>
 ...
 <meta name="description" content="登录明良广CMS {:C('ADMIN_VERSION')}">
 <meta name="keywords" content="登录明良广CMS {:C('ADMIN_VERSION')}">
 <meta name="author" content="jxbh">
 <title>登录明良广CMS</title>
 <link rel="shortcut icon" href="__img__/favicon.ico" />
<style>
body{ font-size:12px;margin:0; padding:0;background:url(__img__/adminbg.jpg) repeat center center; background-attachment:fixed;font-family:"微软雅黑"; }
...
.bhface{ width:140px; height:140px; background:#3d3e3f url('__img__/logo/adminlogo.png') no-repeat center center; background-size:80%; border-radius:50%; border:solid 10px #363839; float:left; margin-right:20px; cursor:pointer; position:relative;}
...
<if condition="$data['adminpath'] eq ''">
 ...
 <!--弹出对话框-->
 <div class="i-alert">
  <div class="i-alert-body"><img src="__img__/sign-icon.png" width="20"> <span class="alert-tips">我是提示</span> <b>&times;</b></div>
 </div>
 <!--结束-->     
 ...
 <script type="text/javascript">
  function gcode() {
	var files = "{:U('Bhadmin/index/logincode','','')}";
  ...
  $(".i-login-btn").click(function(e) {
	...
	$(".i-login-btn").val('登录中....');
    $.post( "{:U('Bhadmin/index/checklogin')}", {'user':user,'pass':pass,'code':code},function(data){
	  $(".i-login-btn").val('登录');
	  var loginTips = '';
	  if(data==1){	
		window.location.href = "{:U('Bhadmin/index/index')}";
		return false;
	  }else if(data==0){
		gcode();
		loginTips = '登录失败，请重试';
 ...
 <else/>
   <div class="lockscreen">
     ...
     <div class="bhlist">
       ...
       <p class="tips">点击<a href="{:U('Home/index/index')}">链接</a>跳转到首页吧。</p>
     </div>
   </div>
