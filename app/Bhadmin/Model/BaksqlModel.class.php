...
class BaksqlModel extends Model {
    public $config = '';                                                        //相关配置
    ...
    public function downloadBak($file_name) {
         if (!file_exists($file_name)) { //检查文件是否存在
            ...
            // 输入文件标签
             ...
             header("Content-Disposition: attachment; filename=" . str_ireplace(C("DB_BACKUP"),'',$file_name));  //以真实文件名提供给浏览器下载
     ...
     */
     ...
     private function setFile() {
		 ...
		 //开始存入数据库
		 $dobj = M('databackup');
		 $dobj->add(array('dataname'=>$fileName,'date'=>dates()));
		 //ending
     }
