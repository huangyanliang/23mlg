...
class FileModel{
  ...
  public function picupload($act=''){//图片上传
    $data    = $this->uploadSet();
	$exts    = ($data['picsuffix']!='') ? explode(',',$data['picsuffix']) : array('jpg', 'gif', 'png', 'jpeg');
	...
	if ( $from == 'meituxiuxiu' || $act == 'editor' ) {
	  ...
	  $uppic->rootPath = $fname.'image/';
	} else {
	  $uppic->rootPath = C('UPLOAD_PATH').'images/';
	}
	$uppic->maxSize  = $maxSize;
	$uppic->exts     = $exts;
	$uppic->subName  = array('date','Ymd');
	$uploadname      = ($act == '') ? 'Filedata' : 'imgFile';
	$info = $uppic->upload();
	if ($info) {
	  $savePath = $info[$uploadname]['savepath'];
	  $saveName = $info[$uploadname]['savename'];
	  ...
	  if ($width > 0 && $height > 0) {
		...
		if ( $from == 'meituxiuxiu' || $act == 'editor' ) {
		  ...
		  if ( ($setsize && $setsize >= $upmaxsize) || $picw>=$upmaxwidth ) {
			...
			$height = intval($cropwidth/$width) * $pich;
		  }
		  if ( $width > 0 && $height > 0 ) {
	  ...
	  return array('file'=>'','error'=>$uppic->getError());
	} 
  } 
   
  private function addwater($imgPath,$iswater=0,$act=''){ //添加水印
	  $data  = $this->uploadSet();
	  if($iswater){
	   return ($imgPath);
	  }
  ...
  private function uploadSet(){
    if (!$uploadField = S('uploadField')) {
	  $data  = M('systemconfig')->field('iswater,fontpos,waterpos,waterpic,fonttext,fontsize,fontcolor,facetype,rotation,wateralpha,picsuffix,filesuffix,picsize,filesize,picmaxwidth,picmaxsize,cropwidth')->where(array('Id'=>1))->find();
	  S('uploadField',$data,3600*24);
	  return $data;
	} else {
	  return $uploadField;
	}
  ...
  public function thumb($h,$w,$file,$act='',$type=1,$savefile=''){
     ...
     if(!file_exists($abspath.$file)){
	   ...
	   //删除掉原图
	   if ( $act == '' ) {
	     @unlink(C("UPLOAD_PATH").'images/'.$file);
	   } else if ( $act == 'editor' ) {
  ...
  public function fileupload($act = '',$uploadType){ //文件上传
    $data    = $this->uploadSet();
	...
	if ( $from == 'meituxiuxiu' || $act == 'editor' ) {
	  ...
	  $upfile->rootPath = $fname.$uploadType.'/';
	  
	} else {
	  $upfile->rootPath = C('UPLOAD_PATH').'files/';
	}
	$upfile->maxSize  = $maxSize;
	if ($exts!='') {
	   $upfile->exts  = $exts;
	}
	$upfile->subName  = array('date','Ymd');
	$uploadname       = ($act == '') ? 'Filedata' : 'imgFile';
	$info = $upfile->upload();
	if ($info) {
	  $savePath = $info[$uploadname]['savepath'];
	  $saveName = $info[$uploadname]['savename'];
	  return array('file'=>$savePath.$saveName,'error'=>'');
	} else {
	  return array('file'=>'','error'=>$upfile->getError());
	} 
  }
  //编辑器上传
  public function editorUpload(){
    $uploadType = I('get.dir');
	if ( $uploadType == 'image' ) {
	  $result = $this->picupload('editor');
	} else {
	  $result = $this->fileupload('editor',$uploadType);
	}
	$abfile   =  __ROOT__.'/public/kindedit/attached/'.$uploadType.'/'; 
	if ( $result['error']!='' ) {
  ...
  public function getpic($spath = '',$page = 1){
	...
	$path     = C("UPLOAD_PATH").'images/'.$spath;
	$data     = $this->getarrays($path);
	$count    = count($data);
	$pageS    = 15;
	$pobj     = new \Think\AjaxPage($count,$pageS,$page);
