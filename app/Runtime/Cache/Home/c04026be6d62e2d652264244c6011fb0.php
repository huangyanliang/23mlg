...
<head>
	...
	<div class="main">
		<div class="addressBox">
			...
			<a href="<?php echo U('download/index');?>">下载专栏</a> > <?php echo ($title); ?>
		</div>
		<div class="menuBox">
			<?php if(is_array($downloadMenu)): $i = 0; $__LIST__ = $downloadMenu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$downloadMenus): $mod = ($i % 2 );++$i;?><a class="hvr-radial-out" <?php if($type == $downloadMenus['Id']): ?>style="background-color:#05B181;color:#fff;"
					<?php else: endif; ?> href="<?php echo U('download/index','type='.$downloadMenus['Id']);?>"><?php echo ($downloadMenus['topic']); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
		</div>
		<ul class="new_ul">
			<?php if(is_array($news)): $i = 0; $__LIST__ = $news;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$newobj): $mod = ($i % 2 );++$i;?><a href="<?php echo U('download/downloadShow','id='.$newobj['Id']);?>"><li><?php echo ($newobj['topic']); ?><span>[<?php echo date('Y-m-d',strtotime($newobj['date']));?>]</span></li></a><?php endforeach; endif; else: echo "" ;endif; ?>
		</ul>
	</div>
