...
<body>
 ...
 <div class="pubmain">
 <h1>类别管理<div class="pull-right"><?php echo btn(array('vals'=>'添加资料','size'=>3,'icon'=>'plus','scene'=>'primary','url'=>U('Product/promtagadd','sty='.$sty.'&tables='.$dshow['table'])));?></div></h1>
 <form name="pubserch" method="get" action=""> 
  <div class="search">
   类别名称：<input type="text" class="text" name="topic" placeholder="类别名称" style="width:150px;">
   &nbsp;所属子类：<?php echo dropdown($infdata,0,'请选择子类','ctag');?>
  ...
  <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="<?php echo tabstyle();?>">
   ...
   <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$obj): $mod = ($i % 2 );++$i;?><tr class="maintr">
    ...
    <td align="left" valign="middle"><?php echo gtopic('proctag',$obj['ctag']);?></td>
    ...
    <td align="center" valign="middle"><?php echo btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','url'=>U('Product/promtagmod','tables='.$dshow['table'].'&id='.$obj['Id'])));?></td>
   ...
   <?php else: ?>
    <div class="alert alert-danger">条件下暂无资料，您可以点击添加按钮添加一条数据。</div><?php endif; ?>
 </div>
