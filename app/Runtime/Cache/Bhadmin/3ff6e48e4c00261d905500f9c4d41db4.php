...
<body>
 ...
 <div class="pubmain">
 <h1>资料管理<div class="pull-right"><?php echo btn(array('vals'=>'添加资料','size'=>3,'icon'=>'plus','scene'=>'primary','url'=>U('Product/productadd','sty='.$sty.'&tables='.$dshow['table'].'&martables='.$dshow['martable'])));?></div></h1>
 <form name="pubserch" method="get" action=""> 
  <div class="search">
   ...
   <div class="pull-left">
   <?php echo dropdownlink($infdata,array('ctag'),array('promtag'));?>
   </div>
  ...
  <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="<?php echo tabstyle();?>">
   ...
   <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$obj): $mod = ($i % 2 );++$i;?><tr class="maintr">
    ...
    <td align="left" valign="middle"><?php echo gtopic('proctag',$obj['ctag']);?> > <?php echo gtopic('promtag',$obj['mtag']);?> </td>
    ...
    <td align="center" valign="middle"><?php echo btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','url'=>U('Product/productmod','tables='.$dshow['table'].'&martables='.$dshow['martable'].'&id='.$obj['Id'])));?></td>
