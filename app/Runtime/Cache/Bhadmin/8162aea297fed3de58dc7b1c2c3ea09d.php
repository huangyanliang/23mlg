...
<body>
 ...
 <div class="pubmain">
  ...
  <div class="pull-right">
   <?php echo btn(array('vals'=>'','size'=>3,'icon'=>'th-list','scene'=>$scene[0],'url'=>U('system/syslogs','act=1')));?>
   <?php echo btn(array('vals'=>'','size'=>3,'icon'=>'folder-open','scene'=>$scene[1],'url'=>U('system/syslogs','act=2')));?>
   <?php echo btn(array('vals'=>'统计','size'=>3,'icon'=>'signal','scene'=>'default','tips'=>'统计文件夹内上传文件大小，文件夹数量，文件数量','dir'=>'bottom','add'=>'mysignal'));?>
  </div></h1>
  <?php if($act == 1): ?><form name="publist" method="post" action="<?php echo U('system/sysdellogs');?>" onSubmit="return pubdel(document.publist)">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="<?php echo tabstyle();?>">
   <tr class="active">
     ...
     <td width="150" align="left" valign="middle"><?php echo icon('time');?> 日志时间</td>
   ...
   <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$obj): $mod = ($i % 2 );++$i;?><tr class="maintr">
    ...
    <td align="left" valign="middle"><a href="javascript:void(0)" class="open-logs" data-path="<?php echo ($obj['file']); ?>"><?php echo btn(array('vals'=>'查看','icon'=>'eye-open','tips'=>'点击查看日志详情'));?></a></td>
   ...
   <tr>
    ...
    <td height="35" align="left" valign="middle" colspan="4">
     ...
     <font class="text-warning">&nbsp;<?php echo icon('warning-sign');?> 提示，任何日志删除都无法找回，建议只删除日期比较前的日志！</font>
     <?php echo ($dshow['pageshow']); ?>
    </td>
   </tr>
  </table>
  </form><?php endif; ?>
  <?php if($act == 2): if(is_array($file)): $i = 0; $__LIST__ = $file;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$fobj): $mod = ($i % 2 );++$i; if($fobj['count'] == 0): ?><a href="javascript:void(0)">
    <?php else: ?>
      <a href="<?php echo U('system/syslogs','path='.$fobj['file']);?>"><?php endif; ?>
    <div class="myfolder">
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content" style="border-radius:0;">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-sm btn-primary" data-dismiss="modal"><?php echo icon('off');?> 关闭</button>
      </div>
    </div>
  </div>
 </div>
