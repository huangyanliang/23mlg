...
<head>
 ...
 <meta name="description" content="登录百恒CMS <?php echo C('ADMIN_VERSION');?>">
 <meta name="keywords" content="登录百恒CMS <?php echo C('ADMIN_VERSION');?>">
 <meta name="author" content="jxbh">
 <title>登录百恒CMS</title>
 <link rel="shortcut icon" href="/bhv3.0/public/Bhadmin/images/favicon.ico" />
<style>
body{ font-size:12px;margin:0; padding:0;background:url(/bhv3.0/public/Bhadmin/images/adminbg.jpg) repeat center center; background-attachment:fixed;font-family:"微软雅黑"; }
div,input{margin:0; padding:0;}
.gear{ width:166px; height:198px; background:url(/bhv3.0/public/Bhadmin/images/gear-line.png) no-repeat center center; position:absolute; z-index:1; left:460px; top:80px; display:none;} 
...
.i-login .i-logindiv{ width:480px; height:auto; margin:0px auto; overflow:hidden;}
.i-login .i-loginhead{ height:55px; width:480px; background:rgba(0,0,0,.5);}
.i-login .i-btn-logo,.i-login .i-btn-login{ width:50%; height:55px; line-height:55px; text-align:center; float:left; color:#888a87; font-size:16px; cursor:pointer;}
...
.i-login .i-user-div .i-user{ width:388px; height:32px; border:solid 1px #d7d7d7; padding:4px 15px; color:#999;}
.i-login .i-code-div .i-code{ width:388px; height:32px; border:solid 1px #d7d7d7; padding:4px 15px; color:#999;border-top:none;}
.i-login #code-verify{ position:absolute; right:2px; top:4px;}
.i-login .i-pass-div .i-pass{ width:388px; height:32px; border:solid 1px #d7d7d7; padding:4px 15px; color:#999; border-top:none;}
.i-login .i-btn-div{ margin:10px auto 0px auto;}
.i-login-btn{ width:420px; height:40px; line-height:40px; background:#54cbf3; color:#fff; text-align:center; border:none; cursor:pointer; font-size:16px;font-family:"微软雅黑"; }
.i-login .i-reg-btn:hover,.i-login-btn:hover{background:#56bee1;}
...
.i-alert{ width:500px; height:40px; background:#fff; position:fixed; top:0%; left:50%; margin-top:10px; margin-left:-250px; z-index:300; border-radius:2px; display:none;}
.i-alert div.i-alert-body{ width:480px; height:40px; margin:0px auto; overflow:hidden; line-height:40px; text-align:left; color:#eb6965;}
.i-alert div.i-alert-body img{ position:relative; top:5px;}
...
@media (max-width:500px){
 ...
 .i-login .i-logindiv,.i-login .i-loginhead,.i-login .i-loginbody,.i-login .i-loginfoot{ width:100%; border-radius:3px;}
...
.bhface{ width:140px; height:140px; background:#3d3e3f url('/bhv3.0/public/Bhadmin/images/logo/adminlogo.png') no-repeat center center; background-size:80%; border-radius:50%; border:solid 10px #363839; float:left; margin-right:20px; cursor:pointer; position:relative;}
...
<?php if($data['adminpath'] == ''): ?><form name="bhadmin" method="post" action="">
 <div class="i-login">
   <div class="logo"><img src="/bhv3.0/public/Bhadmin/images/logo.png"></div>
 ...
 <!--弹出对话框-->
 <div class="i-alert">
  <div class="i-alert-body"><img src="/bhv3.0/public/Bhadmin/images/sign-icon.png" width="20"> <span class="alert-tips">我是提示</span> <b>&times;</b></div>
 </div>
 <!--结束-->     
 <div class="gear"><img src="/bhv3.0/public/Bhadmin/images/gear.png" class="gear-div"></div>
 

 <script type="text/javascript" src="/bhv3.0/public/Bhadmin/js/jquery.min.js"></script>
 <script type="text/javascript">
  function gcode() {
	var files = "<?php echo U('Bhadmin/index/logincode','','');?>";
  ...
  $(".i-login-btn").click(function(e) {
	...
	$(".i-login-btn").val('登录中....');
    $.post( "<?php echo U('Bhadmin/index/checklogin');?>", {'user':user,'pass':pass,'code':code},function(data){
	  $(".i-login-btn").val('登录');
	  var loginTips = '';
	  if(data==1){	
		window.location.href = "<?php echo U('Bhadmin/index/index');?>";
		return false;
	  }else if(data==0){
		gcode();
		loginTips = '登录失败，请重试';
 ...
 <?php else: ?>
   <div class="lockscreen">
     ...
     <div class="bhlist">
       <p class="h2" style="margin-top:35px;">百恒网络提醒您~</p>
