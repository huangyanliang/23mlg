...
<body>
 ...
 <div class="pubmain">
  <h1>类别管理<div class="pull-right"><?php echo btn(array('vals'=>'添加类别','size'=>3,'icon'=>'plus','scene'=>'primary','url'=>U('Product/proctagadd','tables='.$dshow['table'].'&sty='.$sty)));?></div></h1>
  ...
  <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="<?php echo tabstyle();?>">
   <tr class="active">
     <td width="45" align="center" valign="middle" height="37"><?php echo ckall('top');?></td>
  ...
  <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$obj): $mod = ($i % 2 );++$i;?><tr class="maintr">
    ...
    <td align="center" valign="middle"><?php echo date("Y-m-d",strtotime($obj['date']));?></td>
    <td align="center" valign="middle"><?php echo btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','url'=>U('Product/proctagmod','tables='.$dshow['table'].'&id='.$obj['Id'])));?>
    </td>
 ...
 <?php else: ?>
 <div class="alert alert-danger">暂无资料，点击右上角的添加按钮添加一条数据吧</div><?php endif; ?>
 </div>
