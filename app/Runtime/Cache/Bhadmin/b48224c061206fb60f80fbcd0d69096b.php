...
<body>
 ...
 <div class="pubmain">
 <h1>广告管理<div class="pull-right"><?php echo btn(array('vals'=>'添加广告','size'=>3,'faicon'=>'plus','scene'=>'primary','url'=>U('system/advadd')));?></div></h1>
 <?php if($data != ''): ?><form name="pubserch" method="get" action=""> 
  <div class="search">
   广告名称：<input type="text" class="text" name="topic" placeholder="广告名称" style="width:150px;">
   &nbsp;广告位置：<?php echo dropdown($advtype,0,'选择广告位置','ctag');?>
 ...
 <form name="publist" method="post" action="" onSubmit="return pubdel(document.publist)"> 
  <table width="99%" border="0" cellspacing="0" cellpadding="0" align="center" class="<?php echo tabstyle();?>">
   ...
   <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$obj): $mod = ($i % 2 );++$i;?><tr class="maintr">
    ...
    <td align="left" valign="middle"><?php echo modField($obj['topic'],$obj['Id'],'topic','advdata');?></td>
    <td align="left" valign="middle"><?php echo ($obj['ctag'] ? gtopic('advtype',$obj['ctag']) : '--'); ?></td>
    <td align="center" valign="middle"><a href="javascript:void(0)"<?php echo imgshow($obj['pic']);?>><?php echo icon('picture');?></a></td>
    ...
    <td align="center" valign="middle"><?php echo ($obj['date']); ?></td>
    <td align="left" valign="middle"><?php echo btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','url'=>U('system/advmod','tables='.$dshow['table'].'&id='.$obj['Id'])));?></td>
  ...
  <?php else: ?>
    <div class="alert alert-danger">暂无资料，您可以点击添加按钮添加一条数据。</div><?php endif; ?> 
 </div>
