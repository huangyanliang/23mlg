...
<body>
 ...
 <div class="pubmain">
  <h1>类别管理<div class="pull-right"><?php echo btn(array('vals'=>'添加类别','size'=>3,'icon'=>'plus','scene'=>'primary','url'=>U('website/datatypeadd','tables='.$dshow['table'].'&sty='.$sty)));?></div></h1>
  ...
  <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$obj): $mod = ($i % 2 );++$i;?><tr class="maintr">
    ...
    <td align="center" valign="middle"><?php echo btn(array('vals'=>'编辑','icon'=>'edit','tips'=>'点击编辑数据','url'=>U('datatypemod','tables='.$dshow['table'].'&id='.$obj['Id'])));?>
    </td>
