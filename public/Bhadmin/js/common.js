$(document).ready(function(e) {
 ...
 $(".btn-enabled,.btn-disabled").click(function(e) {
  ...
  if( $("#uppic").length>0 ){
    $('#uppic').uploadify({
	    formData : {'iswater' : 0},
		swf      : upload+'/uploadify.swf?v='+Math.random(),
		uploader : abspath+'/fileupload/picupload?v='+Math.random(),
		width    : 400,
		height   : 29,
		fileTypeDesc   : '图片类型',
		...
		onUploadStart : function () {
		   ...
		   $("#uppic").uploadify("settings",'formData',{'iswater':iswater,'width':$(".cutswidth").val(),'height':$(".cutsheight").val()});
		},
  ...
  if ($(".uploadpic").length > 0) {
   $(".uploadpic").each(function(e){
	  ...
	  $(function () {
		$(id).wrap("<form id='"+fromid+"' action='"+abspath+"/fileupload/picupload' enctype='multipart/form-data' method='post'></form>");
		$(id).change(function(){
  ...
  if( $("#upfile").length>0 ){
	$('#upfile').uploadify({
		...
		height   : 29,
		fileTypeDesc    : '文件类型',
		buttonText      : '<span class="glyphicon glyphicon-folder-open"><b>选择上传</b></span>',
